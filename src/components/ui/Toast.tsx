...
export const Toast: React.FC<ToastProps> = ({ toast, onRemove }) => {
  ...
  const getToastStyles = () => {
    const baseStyles = "flex items-start gap-3 p-4 rounded-lg shadow-lg max-w-sm"
          ...
          <div className="flex items-center gap-2">
            {toast.action && (
              <button
                ...
                aria-label={`Ação: ${toast.action.label}`}
              >
                {toast.action.label}
              </button>
            )}
            ...
            <button
              ...
              className="text-sm opacity-70 hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-current rounded p-1"
...
export const useToast = () => {
  ...
  const addToast = (toast: Omit<ToastMessage, 'id'>) => {
    ...
    A11yService.announce(`${toast.type}: ${toast.title}`)
  ...
  // Convenience methods
  const success = (title: string, description?: string, action?: ToastMessage['action']) => {
    return addToast({ type: 'success', title, description, action })
  }

  const error = (title: string, description?: string, action?: ToastMessage['action']) => {
    return addToast({ type: 'error', title, description, action })
  }

  const warning = (title: string, description?: string, action?: ToastMessage['action']) => {
    return addToast({ type: 'warning', title, description, action })
  }

  const info = (title: string, description?: string, action?: ToastMessage['action']) => {
